...
function admin_redirect_success(){
	$dir = "VIEW/html/SME/Index.php?success=1";
	$url = BASE_URL.$dir;
...
function admin_place_redirect($type_of_error){
	...
	$dir = "VIEW/html/SME/Index.php?error=$error_type&User_Product_Id=$Bid_Id&Product_Id=$Product_Id";
	$url = BASE_URL.$dir;
	header("Location:$url");
	exit();
}
	...
	if(TRUE == check_login_status()){
		...
		if($user_type == User_Type::SME){
			...
			if(empty($_POST['User_Name'])){
				$errors[] = "Please refresh the website. user name Should be filled";
			}
			else{
			...
			if(empty($_POST['Amount'])){
				$errors[] = "Please Refresh! The Amount is not Defined";
			}
			else{
				$Amount = $_POST['Amount'];
			}
			...
			if(empty($_POST['Product_Name'])){
				$errors[] = "Please refresh, The product name should be filled";
			}
			else{
				$Product_Name = $_POST['Product_Name'];
			}

			if(empty($_POST['Orderer_Id'])){
				$errors[] = "Orderer Id is not filed";
			}
			else{
			...
			if(empty($_POST['Order_Id'])){
				$errors[] = "order Id should be filled";
			}
			else{
			...
			if(empty($errors)){
				$SME_Con->Accept_Order($Order_Id);
				$Action =  $User_Name." Accepted your ".$Amount." ".$Product_Name." Order.";
				$notification = new Notification($Action,$Orderer_Id);

				if($SME_Con->Add_Notification($notification)){
