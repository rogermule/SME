...
function admin_redirect_success(){
	...
	$dir = "VIEW/html/Admin/SectorManager.php?success=1";
	$url = BASE_URL.$dir;
...
function admin_place_redirect($type_of_error){
	...
	$dir = "VIEW/html/Admin/SectorManager.php?error=$error_type";
	$url = BASE_URL.$dir;
	header("Location:$url");
	exit();
}
	...
	if(TRUE == check_login_status()){
		...
		if($user_type == User_Type::ADMIN){
			...
			if(empty($errors)){
				...
				if($admin_con->Sector_Exists($Sector)){
					admin_place_redirect(Error_Type::SAME_SECTOR_NAME);
				}

				$added = $admin_con->Add_Sector($Sector);
