...
function admin_redirect_success(){
	$dir = "VIEW/html/Admin/Product_Type_Manager.php?success_delete=1";
	$url = BASE_URL.$dir;
...
function admin_place_redirect($type_of_error){
	...
	$dir = "VIEW/html/Admin/Delete_Sector.php?error=$error_type&Product_Type_Id=$Product_Type_Id";
	$url = BASE_URL.$dir;
	header("Location:$url");
	exit();
}
	...
	if(TRUE == check_login_status()){
		...
		if($user_type == User_Type::ADMIN){
			...
			if(empty($_POST['Product_Type_Id'])){
				$errors[] = "Product Type is not Known Please refresh the site!";
			}
			else{
				$Product_Type_Id = $_POST['Product_Type_Id'];
			}
			...
			if(empty($errors)){
				$deleted = $admin_con->Delete_Product_Type($Product_Type_Id);
				if($deleted){
