...
require("../MODEL/Product_Type.php");//this is used to make the product type object
$errors = array();
...
function admin_redirect_success(){
	...
	$dir = "VIEW/html/ADMIN/Product_Type_Manager.php?success=1";
	$url = BASE_URL.$dir;
...
function admin_place_redirect($type_of_error){
	...
	if($type_of_error == Error_Type::FORM){
		...
		$error_type = "Error when adding new user. Fill Out the Form Correctly!";
	}
	else if($type_of_error == Error_Type::SAME_PRODUCT_TYPE_NAME){
		$error_type = "Error! You can Not add same sector name!";
	}
	$dir = "VIEW/html/ADMIN/Product_Type_Manager.php?error=$error_type";
	$url = BASE_URL.$dir;
	header("Location:$url");
	exit();
}
...
//				exit();
			...
			if(empty($errors)){
				...
				$Product_Type = new Product_Type($Product_Type_Name,$Sector_Id);
				if($admin_con->Product_Type_Exists($Product_Type)){
					admin_place_redirect(Error_Type::SAME_PRODUCT_TYPE_NAME);
				}

				$added = $admin_con->Add_Product_Type($Product_Type);
